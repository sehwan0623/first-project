...
class PID_Controller_ConstantTimeGap(object):
    def __init__(self, step_time, target_x, ego_x, ego_vx, timegap=1.5, P_Gain=1.2, D_Gain=3.0, I_Gain=0.1):
        self.timegap = timegap
        self.Kp = P_Gain
        self.Kd = D_Gain
        self.Ki = I_Gain
        self.dt = step_time
        self.e_prev = (target_x - ego_x) - ego_vx * self.timegap
        self.e_sum = 0.0
        self.u = 0.0
        self.space = ego_vx * self.timegap  # 초기 기준 거리

    def ControllerInput(self, target_x, ego_x, ego_vx):
        self.space = ego_vx * self.timegap  # 매 timestep마다 기준거리 갱신
